...
class AttentionQKV(nn.Module):
    ...
    def flash_attention(self, q, k, v, mask=None):
        ...
        # torch.backends.cuda.sdp_kernel was deprecated in PyTorch 2.2+
        try:
            from torch.nn.attention import sdpa_kernel, SDPBackend
            backends = []
            if config.get('enable_flash', False):
                backends.append(SDPBackend.FLASH_ATTENTION)
            if config.get('enable_mem_efficient', False):
                backends.append(SDPBackend.EFFICIENT_ATTENTION)
            if config.get('enable_math', False):
                backends.append(SDPBackend.MATH)
            if not backends:
                backends = [SDPBackend.FLASH_ATTENTION, SDPBackend.EFFICIENT_ATTENTION, SDPBackend.MATH]
            ctx = sdpa_kernel(backends)
        except ImportError:
            ctx = torch.backends.cuda.sdp_kernel(**config)
        with ctx:
