...
class AlignmentStreamAnalyzer:
    ...
    def _add_attention_spy(self, tfmr, buffer_idx, layer_idx, head_idx):
        ...
        # SDPA doesn't support output_attentions — switch to eager attention.
        # Note: output_attentions=True is already passed explicitly in t3.py forward calls,
        # so we only need to change the implementation, not set config.output_attentions.
        if hasattr(tfmr, 'config') and getattr(tfmr.config, '_attn_implementation', None) == 'sdpa':
    ...
    def step(self, logits, next_token=None):
        ...
        # Monotonic masking: prevent attention from looking too far ahead.
        # Once curr_frame_pos exceeds text length S, fall back to text_position-based
        # masking to prevent spurious jumps to end-of-text positions.
        mask_limit = min(max(self.text_position + 6, self.curr_frame_pos + 1), j - i)
        A_chunk[:, mask_limit:] = 0
        ...
        # Require both alignment position near end AND minimum speech frames generated.
        # Each text token needs ~6 speech tokens minimum for natural multilingual speech at 25 tokens/sec.
        # Floor of 150 frames (~6 seconds) prevents premature completion on short text.
        min_speech_frames = max(S * 6, 150)
        self.complete = self.complete or (self.text_position >= S - 2 and self.curr_frame_pos >= min_speech_frames)
        ...
        long_tail = self.complete and (A[self.completed_at:, -3:].sum(dim=0).max() >= 30)  # ~1200ms

        # If there are activations in previous tokens after generation has completed, assume this is a repetition error.
        alignment_repetition = self.complete and (A[self.completed_at:, :-5].max(dim=1).values.sum() > 20)
        ...
        if next_token is not None:
            ...
            # Keep only last 12 tokens to prevent memory issues
            if len(self.generated_tokens) > 12:
                self.generated_tokens = self.generated_tokens[-12:]

        # Check for excessive token repetition (5x same token in a row)
        token_repetition = (
        ...
        # Keep EoS suppressed until alignment is near text end AND enough speech has been generated
        if (cur_text_posn < S - 2 or self.curr_frame_pos < min_speech_frames) and S > 5:
